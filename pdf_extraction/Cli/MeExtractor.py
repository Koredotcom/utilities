...
class Method(Extractor):
    def extract(self):
        ...
        if self.args.get('type') == 'c_ms':
            ...
            assay_title_with_acronym = self.method_e.extract_assay_title_with_acronym()
            if (assay_title_with_acronym.__len__() > 1):
                self.extraction_result.assay_acronym = assay_title_with_acronym[0]
                self.extraction_result.assay_title = assay_title_with_acronym[1]
            else:
                self.extraction_result.assay_title = assay_title_with_acronym[0]
